...
<div class="min-h-screen bg-slate-50 font-sans">
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl pt-12 pb-8">
        <a href="{{ route('employer.jobs.show', $job) }}" class="inline-flex items-center text-sm font-bold text-slate-400 hover:text-indigo-600 transition-colors mb-6">
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl pb-20">
        <form action="{{ route('employer.jobs.update', $job) }}" method="POST" class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 md:p-12 space-y-12">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-8">
                ...
                <div class="space-y-3">
                    ...
                    <textarea name="description" rows="6" required placeholder="Markdown supported..."
                        class="w-full px-6 py-4 rounded-2xl bg-slate-50 border-transparent focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all font-medium text-slate-600 placeholder-slate-400 leading-relaxed">{{ old('description', $job->description) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="relative" x-data="{
                            jobType: '{{ old('job_type', $job->job_type) }}',
                            open: false,
                            options: {
                                'full-time': 'Full-time',
                                'part-time': 'Part-time',
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="space-y-3">
                        ...
                        <input type="number" name="salary_min" value="{{ old('salary_min', $job->salary_min) }}" step="0.01" min="0" placeholder="0.00"
                            class="w-full px-6 py-4 rounded-2xl bg-slate-50 border-transparent focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold text-slate-900 placeholder-slate-400">
                    </div>
                    <div class="space-y-3">
                        <label class="block text-xs font-bold text-slate-900 uppercase tracking-widest">Max Salary</label>
                        <input type="number" name="salary_max" value="{{ old('salary_max', $job->salary_max) }}" step="0.01" min="0" placeholder="0.00"
                            class="w-full px-6 py-4 rounded-2xl bg-slate-50 border-transparent focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold text-slate-900 placeholder-slate-400">
                    </div>
                    <div class="space-y-3">
                         <label class="block text-xs font-bold text-slate-900 uppercase tracking-widest">Currency</label>
                        <input type="text" name="salary_currency" value="{{ old('salary_currency', $job->salary_currency) }}" maxlength="3" placeholder="USD"
                            class="w-full px-6 py-4 rounded-2xl bg-slate-50 border-transparent focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold text-slate-900 placeholder-slate-400">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                     @foreach(['wheelchair_accessible' => 'Wheelchair Accessible', 'sign_language_support' => 'Sign Language Support', 'screen_reader_compatible' => 'Screen Reader Compatible', 'flexible_hours' => 'Flexible Hours', 'remote_work_available' => 'Remote Work Available'] as $field => $label)
                        <label class="flex items-center p-4 bg-slate-50 rounded-xl hover:bg-slate-100 cursor-pointer transition-all border border-transparent hover:border-slate-200">
                            <input type="checkbox" name="{{ $field }}" value="1" {{ old($field, $job->$field) ? 'checked' : '' }}
                ...
                <div class="space-y-3">
                    ...
                    <textarea name="accessibility_accommodations" rows="3"
                        ...
                        placeholder="Describe additional accommodations...">{{ old('accessibility_accommodations', $job->accessibility_accommodations) }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-indigo-50/50 p-8 rounded-[2rem] border border-indigo-50">
                 ...
                 <div class="relative max-w-sm" x-data="{
                    ...
                    <button type="button" 
                        ...
                        <span x-text="options[status] || 'Select Status'" class="uppercase tracking-wide"></span>
            ...
            <div class="flex justify-end items-center gap-6 pt-4">
                <a href="{{ route('employer.jobs.show', $job) }}" class="text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors">Cancel</a>
                <button type="submit" 
