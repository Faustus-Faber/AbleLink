...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-10">
        <div>
            ...
            <p class="text-slate-500 mt-2 font-medium">Update media assets for this course.</p>
        </div>
        <a href="{{ route('admin.courses.edit', $media->course) }}" 
    ...
    <div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-10">
        <form method="POST" action="{{ route('admin.courses.media.update', $media) }}" enctype="multipart/form-data" id="mediaForm">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-8">
                <div x-data="{
                    ...
                    <div class="relative">
                        ...
                        <button type="button" 
                            ...
                            <span x-text="options[kind] || kind"></span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <input name="title" value="{{ old('title', $media->title) }}" placeholder="Display title..."
                    ...
                    <div class="space-y-6">
                         @if($media->media_url)
                            <div class="bg-white rounded-xl p-4 border border-slate-200 flex items-center justify-between">
                                <div>
                                    <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Current Source</span>
                                    <div class="mt-1">
                                        <a class="font-bold text-blue-600 hover:text-blue-800 hover:underline text-sm break-all" href="{{ $media->media_url }}" target="_blank" rel="noreferrer">
                                            {{ $media->external_url ?: 'View Uploaded File' }}
                                        </a>
                                    </div>
                                </div>
                                @if($media->media_path)
                                    <div class="flex items-center gap-3 bg-red-50 p-2 rounded-lg border border-red-100/50">
                                        <input id="remove_media_file" name="remove_media_file" type="checkbox" value="1"
                                               class="w-4 h-4 rounded border-red-200 text-red-600 focus:ring-red-500" />
                                        <label for="remove_media_file" class="text-xs font-bold text-red-700 cursor-pointer select-none">Remove File</label>
                                    </div>
                                @endif
                            </div>
                        @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                        <div>
                            ...
                            @if($media->captions_path)
                                <div class="bg-slate-50 rounded-lg p-3 mb-3 border border-slate-100 flex items-center justify-between">
                                    <span class="text-xs font-medium text-slate-600">File attached</span>
                                    <div class="flex items-center gap-2">
                                        <a href="{{ $media->captions_url }}" target="_blank" class="text-xs font-bold text-blue-600 hover:underline">Download</a>
                                        <div class="h-3 w-px bg-slate-300"></div>
                                        <div class="flex items-center gap-1.5">
                                            <input id="remove_captions_file" name="remove_captions_file" type="checkbox" value="1"
                                                class="w-3.5 h-3.5 rounded border-red-200 text-red-600 focus:ring-red-500" />
                                            <label for="remove_captions_file" class="text-[10px] font-bold text-red-600 uppercase">Remove</label>
                                        </div>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div>
                            ...
                            <input name="captions_language" value="{{ old('captions_language', $media->captions_language ?: 'en') }}" placeholder="en"
                    ...
                    <div class="space-y-6">
                            ...
                            <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Audio Description Track</label>
                             @if($media->audio_description_path)
                                <div class="bg-slate-50 rounded-lg p-3 mb-3 border border-slate-100 flex items-center justify-between">
                                    <span class="text-xs font-medium text-slate-600">File attached</span>
                                    <div class="flex items-center gap-2">
                                        <a href="{{ $media->audio_description_url }}" target="_blank" class="text-xs font-bold text-blue-600 hover:underline">Download</a>
                                        <div class="h-3 w-px bg-slate-300"></div>
                                        <div class="flex items-center gap-1.5">
                                            <input id="remove_audio_description_file" name="remove_audio_description_file" type="checkbox" value="1"
                                                class="w-3.5 h-3.5 rounded border-red-200 text-red-600 focus:ring-red-500" />
                                            <label for="remove_audio_description_file" class="text-[10px] font-bold text-red-600 uppercase">Remove</label>
                                        </div>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div>
                            ...
                            <textarea name="transcript" rows="6" placeholder="Paste full transcript here..."
                                      class="w-full bg-slate-50 border-transparent focus:bg-white focus:border-slate-300 focus:ring-4 focus:ring-slate-100 rounded-xl px-4 py-3 font-medium text-slate-900 transition-all resize-y placeholder:text-slate-400">{{ old('transcript', $media->transcript) }}</textarea>
                        </div>
                    </div>
                </div>
    ...
    <div class="mt-8 bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-8">
        <div class="flex items-center justify-between">
            <div>
                 <h2 class="text-xl font-bold text-slate-900">Delete Media</h2>
                 <p class="text-slate-500 text-sm mt-1">Permanently remove this media item and its files.</p>
            </div>
           
            <form method="POST" action="{{ route('admin.courses.media.destroy', $media) }}"
                  onsubmit="return confirm('Delete this media item? This action cannot be undone.');">
                @csrf
                @method('DELETE')
                <button type="submit"
                        class="px-6 py-3 rounded-xl bg-red-600 text-white font-bold text-sm hover:bg-red-700 transition-all shadow-sm shadow-red-600/20">
                    Delete
                </button>
            </form>
        </div>
